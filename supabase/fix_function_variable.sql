-- FIX: Corrected Registration Function
-- Fixes the 'record not assigned yet' error by using explicit variables instead of a RECORD type
...
BEGIN
  ...
  LOOP
    ...
    -- FIXED: Selecting into individual variables instead of uninitialized RECORD fields
    SELECT registered_count, capacity, date, shift_name 
    INTO v_current_count, v_capacity, v_date, v_shift_name
    FROM shift_slots 
    WHERE id = v_slot_id 
    FOR UPDATE;
    ...
    IF v_current_count >= v_capacity THEN
      v_unavailable_slots := array_append(
        v_unavailable_slots, 
        json_build_object('date', v_date, 'shift', v_shift_name)
      );
    END IF;
  END LOOP;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Return actual error for debugging if something else goes wrong
